...
#endif
    MENU_ITEM(submenu, MSG_VERSION, lcd_version);
    END_MENU();
}
...
static void lcd_light_menu()
{
    ...
    MENU_ITEM(gcode, MSG_LIGHTS_ON, PSTR("M42 P5 S255"));
    MENU_ITEM(gcode, MSG_LIGHTS_OFF, PSTR("M42 P5 S0"));
    END_MENU();
}

static void extrudio()
{
...
{
    menu_action_gcode(PSTR("G92 E75"));
    delay(2);
...
//    MENU_ITEM(gcode, MSG_EXTRUDE, pgcode("G92 E0 \nG1 E80 F500"));
    
    END_MENU();
}

static void lcd_temperature_settings_menu()
{
    START_MENU();
    MENU_ITEM(back, MSG_CONTROL, lcd_control_menu);
...
static void lcd_delta_calibrate_menu()
{
    ...
    MENU_ITEM(back, MSG_PREPARE, lcd_prepare_menu);
    MENU_ITEM(gcode, MSG_AUTO_HOME, PSTR("G28"));
    MENU_ITEM(gcode, "home and calibrate", PSTR("G28\nG29"));
...
static void lcd_control_menu()
{
    START_MENU();
    MENU_ITEM(back, MSG_MAIN, lcd_main_menu);
    MENU_ITEM(submenu, MSG_TEMPERATURE, lcd_temperature_settings_menu);
    MENU_ITEM(submenu, MSG_MOTION, lcd_control_motion_menu);
    MENU_ITEM(submenu, MSG_LIGHTS, lcd_light_menu);
